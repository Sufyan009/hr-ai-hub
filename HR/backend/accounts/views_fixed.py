...
from .models import (
    Candidate, JobTitle, City, Source, CommunicationSkill, 
    JobPost, ChatSession, ChatMessage, Note, Notification
)
from .serializers import (
    CandidateSerializer, JobTitleSerializer, CitySerializer, 
    SourceSerializer, CommunicationSkillSerializer, JobPostSerializer,
    ChatSessionSerializer, ChatMessageSerializer, NoteSerializer,
    NotificationSerializer
)
# IsAdminOrRecruiter is defined below in this file
from .utils import MessageCompressor, MessageCache, PerformanceOptimizer, AnalyticsHelper
import logging
...
class UserSettingsView(APIView):
    ...
    def get(self, request):
        try:
            ...
            logger.error(f"Error in UserSettingsView.get: {e}")
            return Response({'error': 'Failed to retrieve user settings'}, status=500)
    ...
    def patch(self, request):
        try:
            ...
            logger.error(f"Error in UserSettingsView.patch: {e}")
            return Response({'error': 'Failed to update user settings'}, status=500)
...
class CommunicationSkillSerializer(ModelSerializer):
    class Meta:
        ...
        fields = ['id', 'name']
class CommunicationSkillViewSet(viewsets.ModelViewSet):
    # type: ignore[attr-defined]
...
def all_candidates_view(request):
    ...
    # Get all candidates without pagination
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def candidate_vector_search_view(request):
    """Vector search for candidates using semantic embeddings"""
    ...
    try:
        ...
        results = vector_db.search_candidates(query, limit=limit)
        ...
        if results:
            ...
            for result in results:
                candidate_data = {
                    ...
                    'candidate_stage': result.get('candidate_stage', ''),
            ...
            return Response({
                ...
                'count': 0,
                'query': query,
                'message': f'No candidates found for query: {query}'
            })
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def schema_org_data_view(request):
    """
    API endpoint to expose Schema.org structured data for AI consumption.
    Returns structured data for candidates and job posts in Schema.org format.
    """
    data_type = request.GET.get('type', 'candidates')  # 'candidates' or 'job_posts'
    candidate_id = request.GET.get('candidate_id')
    job_post_id = request.GET.get('job_post_id')
    
    if data_type == 'candidates':
        if candidate_id:
            # Return specific candidate's Schema.org data
            try:
                candidate = Candidate.objects.get(id=candidate_id)
                schema_data = candidate.get_schema_org_data()
                return Response({
                    'success': True,
                    'data': schema_data,
                    'type': 'Person',
            ...
            # Return Schema.org data for all candidates
            candidates = Candidate.objects.all()[:50]  # Limit to prevent large responses
            schema_data_list = []
            ...
            return Response({
                'success': True,
                'data': schema_data_list,
                'type': 'Person[]',
                'context': 'Job Applications',
                'count': len(schema_data_list)
            })
    
    elif data_type == 'job_posts':
        if job_post_id:
            # Return specific job post's Schema.org data
            try:
                job_post = JobPost.objects.get(id=job_post_id)
                schema_data = job_post.get_schema_org_data()
                return Response({
                    'success': True,
                    'data': schema_data,
                    'type': 'JobPosting',
            ...
            # Return Schema.org data for all job posts
            job_posts = JobPost.objects.all()[:50]  # Limit to prevent large responses
            schema_data_list = []
        ...
        return Response({
            'success': False,
            'message': 'Invalid data type. Use "candidates" or "job_posts"'
        }, status=400) 